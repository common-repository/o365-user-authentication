 ...
 */
class O365_USER_AUTH_ONLINE_AuthorizationHelper
{
	...
	public static function validateIdToken($id_token, $settings, $antiforgery_id) {
		...
		foreach ($discovery->keys as $key) {
			try {
				...
				$o365_online_JWT = o365_online_JWT::decode($id_token, $key_pem, self::$allowed_algorithms);
				break;
			} catch (Exception $e) {
				$lastException = $e;
			}
		}
		if ($o365_online_JWT == NULL) {
			throw $lastException;
		}
		if ($o365_online_JWT->nonce != $antiforgery_id) {
			throw new DomainException(sprintf('Nonce mismatch. Expecting %s', $antiforgery_id));
		}
		return $o365_online_JWT;
	}
}