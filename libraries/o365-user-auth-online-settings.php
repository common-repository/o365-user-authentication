<?php
/**
 * Class containing all settings used by the Azure user login.
 *
 ...
 */
class O365_USER_AUTH_Settings_ONLINE {
	/**
	 * @var \O365_USER_AUTH_Settings $instance The settings instance.
	 */
	...
	*/
	 ...
	 */
	public $resourceURI = 'https://graph.microsoft.com';
	/**
	...
	*/
	public static function loadSettingsFromJSON() {
		$settings = self::getInstance();
		
		// Import from Settings.json
		$settings->importSettingsFromJSON('','setting');
		// Import from openid-configuration
		$settings->importSettingsFromJSON($settings->openid_configuration_endpoint,'settings');
		return $settings;
	}
	function importSettingsFromJSON($sourcefile,$checkkey) {
		// Load the JSON settings
		if( $checkkey == 'settings' )
		{
			$jsonSettings = file_get_contents($sourcefile);
		}
		else if( $checkkey == 'setting' )
		{
			$jsonSettings = get_option( 'azure_login_setting_flow','default-value' );
		}
		
		if( $jsonSettings != "default-value"){
